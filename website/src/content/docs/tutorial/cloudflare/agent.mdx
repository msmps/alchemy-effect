...
The [AI Gateway tutorial](/tutorial/cloudflare/ai-gateway) wired a
...
The same `aiGateway.model({...})` call from the previous tutorial
...
- `DurableObjectChatPersistence` — produces a `BackingPersistence`
  from `state.storage`.
- `Chat.layerPersisted({ storeId: "chat" })` — produces a
...
- A `Chat.Persistence` layered on top of `state.storage` via
  `DurableObjectChatPersistence` — drop-in compatible with

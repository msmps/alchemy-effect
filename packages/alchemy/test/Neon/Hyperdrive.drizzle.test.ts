...
import HyperdriveWorker from "./fixtures/hyperdrive-drizzle/worker.ts";
import type { Widget } from "./fixtures/hyperdrive-drizzle/schema.ts";
...
describe.skipIf(!process.env.NEON_TEST)(() => {
   ...
   * the Drizzle Effect client over real Postgres via a Worker.
   */
  test.provider(
    "Neon.Branch + Hyperdrive + Drizzle round-trips through a Worker",
    (stack) =>
      Effect.gen(function* () {
        yield* stack.destroy();

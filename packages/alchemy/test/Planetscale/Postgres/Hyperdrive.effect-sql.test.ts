...
describe.skipIf(!process.env.PLANETSCALE_TEST)(() => {
  /**
   * Same shape as the Drizzle variant, but the Worker uses
   * `@effect/sql-pg`'s `PgClient.layer` + `Layer.unwrapEffect` to derive
   * the SQL client from `hd.connectionString` lazily. Validates that the
   * tutorial's raw-SQL path actually round-trips INSERT/SELECT/DELETE
   * through Hyperdrive against a real Planetscale Postgres role.
   */
  test.provider(
    "PostgresBranch + Hyperdrive + @effect/sql-pg round-trips through a Worker",
    (stack) =>
      Effect.gen(function* () {
        yield* stack.destroy();

...
/**
 * Worker fixture mirroring the Drizzle variant but written against
 * `@effect/sql-pg` directly — exercises Cloudflare.Hyperdrive over
 * Planetscale Postgres via raw `sql\`...\`` templates.
 */
export default class HyperdriveWorker extends Cloudflare.Worker<HyperdriveWorker>()(
  "PlanetscalePgEffectSqlWorker",
  {
      ...
      fetch: Effect.gen(function* () {
        ...
        if (request.method === "GET" && url.pathname === "/widgets") {
          const widgets = yield* sql<Widget>`
            SELECT id, name FROM alchemy_postgres_widgets_sql ORDER BY id
          `;
        ...
        if (request.method === "POST" && url.pathname === "/widgets") {
          ...
          const [inserted] = yield* sql<Widget>`
            INSERT INTO alchemy_postgres_widgets_sql (id, name)
        ...
        if (request.method === "DELETE" && idMatch) {
          ...
          const [deleted] = yield* sql<Widget>`
            DELETE FROM alchemy_postgres_widgets_sql
            WHERE id = ${id}
            RETURNING id, name
          `;
